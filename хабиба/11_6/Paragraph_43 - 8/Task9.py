...
'''
    9. Перепишите программу для поиска количества путей в графе так, чтобы она не вычисляла данные повторно. 
    Например, если мы подсчитали количество путей из вершины X в конечную вершину, то когда это значение 
    потребуется снова, нужно сразу взять полученный ранее результат, а не вызывать функцию рекурсивно.
'''
